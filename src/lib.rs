...
pub const CURRENT_BLOCK_HEIGHT: u64 = 951_173;
...
pub fn is_mainnet(network: &str) -> bool {
    ...
    let network: &str = &network.to_lowercase();
    network == "mainnet"
}
...
pub fn is_same_wallet<T>(wallet1: &T, wallet2: &T) -> bool {
    ...
    let same_wallet: bool = std::ptr::eq(wallet1, wallet2);
    same_wallet
}
...
pub fn normalize_address(address: &str) -> String {
    ...
    let normalized_address: String = address.trim().to_lowercase();
    normalized_address
}
...
pub fn add_utxo(utxos: Vec<Utxo>, new_utxo: Utxo) -> Vec<Utxo> {
    ...
    let mut utxos_: Vec<Utxo> = utxos;
    utxos_.push(new_utxo);
    utxos_
}
...
pub fn find_high_fee(fee_list: &[f64]) -> Option<(usize, f64)> {
    ...
    let mut high_fee: Option<(usize, f64)> = None;
...
pub fn get_wallet_details() -> (String, f64) {
    ...
    let name: String = "satoshi_wallet".to_lowercase().to_string();
    let balance: f64 = 50.0;
    let wallet_dets: (String, f64) = (name, balance);
    wallet_dets
}
...
pub fn unpack_wallet_info(wallet_info: (String, f64)) -> String {
    ...
    let (name, balance): (String, f64) = wallet_info;
...
pub fn calculate_sats(btc: f64) -> u64 {
    ...
    let sats: u64 = (btc * BTC_TO_SATS as f64) as u64;
    sats
}
...
pub fn find_utxo_with_min_value(utxos: &[Utxo], target: u64) -> Option<Utxo> {
    ...
    let mut min_utxo: Option<Utxo> = None;
    let mut min_value: u64 = u64::MAX;

    for utxo in utxos {
        if utxo.value >= target && utxo.value < min_value {
            min_value = utxo.value;
            min_utxo = Some(utxo.clone());
        }
    }
    min_utxo
}
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    let decodedtx =
        hex::decode(raw_tx_hex).map_err(|_e: hex::FromHexError| format!("Hex decode error"))?;
    if decodedtx.len() < 4 {
        return Err("Transaction data too short".to_string());
    }

    let version_bytes: [u8; 4] = decodedtx[0..4]
        .try_into()
        .expect("Failed to extract version bytes");
